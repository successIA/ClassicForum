...
@register.simple_tag
def thread_url(thread_absolute_url, comment_count, new_comment_id, new_comment_count):
    try:
...
def get_category_list(category=None):
    if category:
        return {"category_list": Category.objects.all(), "current_category": category}
...
def get_thread_filter_dropdown(current_thread_filter, is_auth=False, category=None):
    filter_list = None
    if is_auth:
        filter_list = [
            ("new", "New"),
...
def url_with_page_num(url, page_number):
    return "%s?page=%s#post-form" % (url, page_number)
