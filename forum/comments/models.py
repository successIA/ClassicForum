...
class Comment(TimeStampedModel):
    ...
    mentioned_users = models.ManyToManyField(
        settings.AUTH_USER_MODEL, blank=True, related_name="comment_mention"
    )
    ...
    def set_message(self):
        ...
        self.marked_message = get_rendered_message(self.message, user_value_list)
    ...
    def hide(self, *args, **kwargs):
        self.thread.comments.pure().filter(pk__gt=self.pk).update(
            offset=F("offset") - 1
        )

        self.thread.comments.pure().filter(pk=self.pk).update(visible=False)
    ...
    def get_like_url(self):
        return reverse(
            "comments:like", kwargs={"thread_slug": self.thread.slug, "pk": self.pk}
        )
    ...
    def get_url_for_next_or_prev(self):
        ...
        if self.position > 1:
            next_obj_qs = Klass.objects.active().filter(pk__gt=self.pk).order_by("pk")
            if next_obj_qs:
                return next_obj_qs.first().get_precise_url()
            else:
                prev_obj_qs = (
                    Klass.objects.active().filter(pk__lt=self.pk).order_by("pk")
                )
                if prev_obj_qs:
...
class CommentRevision(models.Model):
    ...
    marked_message = models.TextField(max_length=4000, blank=True)
    mentioned_users = models.ManyToManyField(
        settings.AUTH_USER_MODEL, blank=True, related_name="comment_rev_mention"
    )
