...
def get_bbcode_message_quote(parent_comment):
    open_quote_tag = (
        f'[quote="{parent_comment.user.username}, comment:{parent_comment.id}"]\n'
    )
    close_quote_tag = "\n[/quote]\n"
...
def get_user_value_list(user_list):
    return [
        {"username": usr.username, "url": usr.get_absolute_url()} for usr in user_list
    ]


def get_mentioned_users_in_message(message):
    mentions = find_mentioned_usernames(message)
    mentioned_user_list = list(User.objects.filter(username__in=mentions).all())
...
def perform_comment_save(comment, prev_msg=None):
    is_pure_comment_create = (
        True if not comment.pk and not comment.is_starting_comment else False
    )

    if is_pure_comment_create:
        comment.set_position()
...
def _perform_comment_post_save_actions(comment, prev_msg, mentions):
    ...
    notif = Notification(
        sender=comment.user, comment=comment, notif_type=Notification.USER_MENTIONED
    )
