...
class CommentCreateViewWithMentionTest(CommentViewsTest):
    ...
    def test_post_with_repeated_mentions(self):
        self.make_user("first_mentionee")  # first_mentionee
        self.make_user("second_mentionee")  # second_mentionee
        self.make_user("mistake_mentionee")  # mistake_mentionee
        self.make_user("invalid_mentionee")  # invalid_mentionee
        message = """
...
class CommentUpdateViewTest(CommentViewsTest):
    def setUp(self):
        ...
        self.comment = Comment.objects.create(
            message="hello world", thread=self.thread, user=self.user
        )
...
class CommentReplyViewTest(CommentViewsTest):
    ...
    def test_anonymous_user_redirect(self):
        """An anonymous user should be redirected to the login page"""
        redirect_url = "%s?next=%s" % (reverse("accounts:login"), self.reply_url)
...
class CommentLikeTest(CommentViewsTest):
    ...
    def test_anonymous_user_redirect(self):
        """An anonymous user should be redirected to the login page"""
        redirect_url = "%s?next=%s" % (reverse("accounts:login"), self.like_url)
