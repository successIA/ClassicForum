...
def get_filtered_threads_for_page(request, filter_str=None, thread_qs=None, user=None):
...
def _get_filtered_threads(request, filter_str=None, thread_qs=None, user=None):
...
def get_additional_thread_detail_ctx(request, thread, form_action):
    ...
    if not form_action:
        form_action = thread.get_comment_create_form_action(comment_paginator.number)
...
def perform_thread_post_create_actions(thread):
    ...
    notif = Notification(
        sender=thread.user, thread=thread, notif_type=Notification.THREAD_CREATED
    )
    Notification.objects.notify_users(notif, thread.user.followers.all())


def perform_thread_post_update_actions(thread):
    followers = get_user_list_without_creator(thread.followers.all(), thread.user)

    notif = Notification(
        sender=thread.user, thread=thread, notif_type=Notification.THREAD_UPDATED
    )
