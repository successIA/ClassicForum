...
class Thread(TimeStampedModel, HitCountMixin):
    ...
    hit_counts = GenericRelation(
        HitCount, object_id_field="object_pk", related_query_name="threads"
    )
    ...
    def synchronise(self, comment, added=True):
        ...
        self.save(
            update_fields=["final_comment_user", "final_comment_time", "comment_count"]
        )

    def set_starting_comment(self, comment):
        self.starting_comment = comment
        self.final_comment_time = comment.created
        self.save(update_fields=["starting_comment", "final_comment_time"])
    ...
    def get_comment_create_form_action(self, page_num):
        return "%s?page=%s#post-form" % (
            reverse("comments:comment_create", kwargs={"thread_slug": self.slug}),
            page_num,
        )
...
class ThreadFollowershipQuerySet(models.query.QuerySet):
    ...
    def synchronise(self, thread, comment):
        ...
        self.filter(
            user__in=user_list,
            thread=thread,
        ).update(new_comment_count=F("new_comment_count") + 1)
        self.filter(
            user__in=user_list, thread=thread, first_new_comment__isnull=True
        ).update(first_new_comment=comment)
    ...
    def _get_next_first_new_comment(self, comments):
        ...
        if comments.has_next():
            comment_qs = Comment.objects.filter(
                thread=self.thread, created__gt=self.first_new_comment.created
...
class ThreadRevision(models.Model):
    ...
    starting_comment = models.ForeignKey(
        ...
        blank=True,
        null=True,
        related_name="starting_thread_revision",
    )
