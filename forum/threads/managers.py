...
class ThreadQuerySet(models.query.QuerySet):
    ...
    def get_for_user(self, request):
        ...
        queryset2 = (
            queryset.get_related()
            .exclude(followers=request.user)
            .annotate(
    ...
    def get_recent(self, request):
        return self.get_for_user(request).order_by("-final_comment_time")
    ...
    def get_new_for_user(self, request):
        ...
        queryset = (
            queryset.get_related()
            .filter(followers=request.user, threadfollowership__new_comment_count__gt=0)
            .annotate(
                new_c_id=F("threadfollowership__first_new_comment"),
                new_c_num=F("threadfollowership__new_comment_count"),
            )
    ...
    def get_only_for_user(self, request):
        ...
        queryset1 = (
            queryset.get_related()
            .filter(user=request.user, threadfollowership__user=request.user)
            .annotate(
                new_c_id=F("threadfollowership__first_new_comment"),
                new_c_num=F("threadfollowership__new_comment_count"),
            )
        ...
        queryset2 = (
            ...
            .exclude(threadfollowership__user=request.user)
            .annotate(
                new_c_id=Value("0", output_field=CharField()),
                new_c_num=Value("0", output_field=CharField()),
            )
    ...
    def get_recent_for_user(self, request, user, count=5):
        ...
        if is_auth and request.user.is_owner(user):
            return self.get_only_for_user(request).order_by("-final_comment_time")[
                :count
            ]
        return (
            self.get_related().filter(user=user).order_by("-final_comment_time")[:count]
        )
