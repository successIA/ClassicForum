...
# Match ] and as many whitespace character if present and a compulsory new line character  # noqa: E501
...
def get_parent_comment_info(comment_info_dict):
    ...
    try:
        ...
        if username and comment_id:
            ...
            comment_list = list(
                Comment.objects.filter(pk=comment_id, user__username=username)
            )
...
def replace(match):
    ...
    result = re.search(open_tag_capture_regex, match.group(2), re.DOTALL)
    if result and result.groupdict():
        ...
        if comment and username:
            ...
            return (
                f'<aside class="quote"><blockquote>{marked_text}</blockquote></aside>'
            )
...
def bbcode_quote(text):
    ...
    placeholder_start_tag_count = len(re.findall(open_tag_capture_regex, text))
    ...
    tag_length = (
        start_tag_total_count
        if (start_tag_total_count <= end_tag_count)
        else end_tag_count
    )

    found_any_match = False
    for count in range(tag_length):
        placeholder_start_tag_test = len(re.findall(open_tag_capture_regex, text)) > 0

        start_tag_test = len(re.findall(open_tag_regex, text)) > 0
        end_tag_test = len(re.findall(close_tag_regex, text)) > 0

        if (placeholder_start_tag_test and end_tag_test) or (
            start_tag_test and end_tag_test
        ):
