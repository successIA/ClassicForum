...
class NotificationQuerySet(models.query.QuerySet):
    ...
    def mark_as_read(self, receiver, notif_id_list):
        if notif_id_list:
            self.get_for_user(receiver).filter(
                unread=True, id__in=notif_id_list
            ).update(unread=False, modified=timezone.now())

    def get_receiver_url_and_count(self, receiver):
        qs_receiver = self.filter(receiver=receiver).select_related("receiver")
        unread_qs = list(qs_receiver.filter(unread=True).order_by("-created"))
        position = 0
        url = ""
    ...
    def get_for_user(self, user):
        return (
            self.select_related("sender", "receiver", "thread", "comment")
            .prefetch_related("thread__starting_comment", "comment__thread")
...
class Notification(TimeStampedModel):
    ...
    NOTIF_COMMENT_TYPES = [COMMENT_LIKED, COMMENT_REPLIED, USER_MENTIONED]
    NOTIF_USER_TYPES = [USER_FOLLOWED]

    sender = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="sender_notif"
    )
    ...
    def _validate_fields_or_error(self):
        ...
        if (
            ...
            and self.notif_type not in Notification.NOTIF_USER_TYPES
        ):
            raise FieldError("Invalid notification type")

    def get_description(self):
        return render_to_string("notifications/notification.html", {"notif": self})
