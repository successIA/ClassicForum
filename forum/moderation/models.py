...
class Moderator(TimeStampedModel):
    ...
    def is_supermoderator_to(self, obj):
        ...
        return (
            self.user.is_supermoderator
            and self.categories.filter(pk__in=obj_pk_list).exists()
        )
    ...
    def can_hide_post(self, post):
        is_starting_comment = isinstance(post, Comment) and post.is_starting_comment
        if post.visible and self.is_moderating_post(post) and not is_starting_comment:
    ...
    def get_post_hide_action_url(comment):
        if comment.is_starting_comment:
            return reverse(
                "moderation:thread_hide", kwargs={"slug": comment.thread.slug}
            )
        else:
            return reverse(
