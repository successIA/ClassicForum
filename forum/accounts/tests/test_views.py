...
class UserProfileStatsViewTest(TestCase):
    def test_stats(self):
        ...
        url = reverse("accounts:user_stats", kwargs={"username": user.username})
        ...
        self.assertIsInstance(response.context["recent_comments"], CommentQuerySet)
        self.assertIsInstance(response.context["recent_threads"], ThreadQuerySet)
...
class UserNotificationList(TestCase):
    ...
    def test_anonymous_user_redirect(self):
        ...
        redirect_url = "%s?next=%s" % (reverse("accounts:login"), self.notif_url)
    ...
    def test_authenticated_user_with_permission(self):
        ...
        self.assertEqual(response.context["current_profile_page"], "user_notifs")
...
class UserProfileEditTest(TestCase):
    ...
    def test_render_for_authenticated_user_with_permission(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context["userprofile"], self.user)
        self.assertEqual(response.context["current_profile_page"], "settings")
...
class UserCommentListTest(TestCase):
    def test_list(self):
        user = self.make_user("testuser1")
        url = reverse("accounts:user_comments", kwargs={"username": user.username})
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context["userprofile"], user)
...
class UserThreadListTest(TestCase):
    ...
    def test_list(self):
        ...
        for filter_str in ["new", "following", "me"]:
            ...
            self.assertEqual(response.status_code, 200)
            self.assertEqual(response.context["userprofile"], user)
            self.assertEqual(response.context["current_profile_page"], filter_str)
...
class FollowUserTest(TestCase):
    def setUp(self):
        ...
        self.url1 = reverse(
            "accounts:user_follow", kwargs={"username": self.user1.username}
        )
        self.url2 = reverse(
            "accounts:user_follow", kwargs={"username": self.user2.username}
        )

    def test_anonymous_user_redirect(self):
        """An anonymous user should be redirected to the login page"""
        redirect_url = "%s?next=%s" % (reverse("accounts:login"), self.url1)
...
class UserFollowingTest(TestCase):
    def test_render(self):
        user = self.make_user("testuser1")
        url = reverse("accounts:user_following", kwargs={"username": user.username})
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context["userprofile"], user)
        self.assertEqual(response.context["current_profile_page"], "user_following")
...
class UserFollowersTest(TestCase):
    def test_render(self):
        user = self.make_user("testuser1")
        url = reverse("accounts:user_followers", kwargs={"username": user.username})
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context["userprofile"], user)
        self.assertEqual(response.context["current_profile_page"], "user_followers")
