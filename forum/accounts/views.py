...
def user_profile_stats(request, username):
    ...
    active_category = comment_qs.get_user_active_category(user, comment_count)
    ctx = {
        "userprofile": user,
...
def user_mention(request):
    ...
    user_queryset = User.objects.filter(username__istartswith=username).order_by(
        "username"
    )[:8]
    if username:
...
def user_mention_list(request):
    ...
    if username_dict_list:
        username_list = [
            username_dict["username"] for username_dict in username_dict_list
        ]
        user_qs = User.objects.filter(username__in=username_list)
        if user_qs.exists():
